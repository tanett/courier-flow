...
import { usePatchProductMutation } from '../../../entities/products/api/api';
import { createInitialFormHelper } from '../../../entities/products/helpers/createInitialForm';
import { PRODUCT_ADDITIONAL_FIELD, typeProductAdditionalFieldInfo, typeProductEdit } from '../../../entities/products/model/state-slice';
...
import useGetProductDataByIdFromUrl from '../../../entities/products/hooks/use-get-product-data-by-id-from-url';
...
export const ProductFormEdit: React.FC<{
    ...
    const onSave = async () => {
        ...
        if (form.values.unit && productData) {

            setIsInProgress(true);

            const dataObject: typeProductEdit = {
                id: productData.id,
                barcodes: getBarcodesValueForEdit(form.values.barcodes, productData.barcodes),
            ...
            try {
                ...
                await editProduct(dataObject).unwrap();

                dispatchAppT(notificationActions.addNotification({
                    type: NOTIFICATION_TYPES.SUCCESS,
                    message: i18n._(t`Product edited successfully.`),
                }));
