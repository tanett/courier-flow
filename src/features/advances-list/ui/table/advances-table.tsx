                    ...
                    { advancesList.length > 0 && advancesList.map((item, index) => {
                        ...
                        return (
                            <Table.Tr key={ item.id } handler={ () => goToDetailsPage(item.id) }>
                                ...
                                <Table.Td><Box sx={ { minWidth: rem(170) } }>{ 'item.employee' }</Box></Table.Td>
                                <Table.Td align={'center'}><Flex justify={'center'} sx={ { maxWidth: rem(132) } }>{ item.productCount }</Flex></Table.Td>
